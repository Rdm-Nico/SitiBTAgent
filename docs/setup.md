...
vLLM invece utilizza un file bash che avvia un podman pod. Questo file è presente direttamente nella macchina con la GPU è _deve_ essere avviato ogni giorno(il processo è automaticamete gestito da crontab). Maggiore informazioni per avviare il pod li puoi leggere [qui](/production/podman/gpu-machine/README.md).
